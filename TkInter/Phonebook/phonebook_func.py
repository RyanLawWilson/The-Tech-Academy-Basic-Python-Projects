    ...
    with conn:
        ...
        if count < 1:
            cur.execute("\
                ...
                (col_fname, col_lname, col_fullname, col_phone, col_email) \
                VALUES \
...
def onSelect(self,event):
    ...
    with conn:
        cursor = conn.cursor()
        cursor.execute("\
            SELECT col_fname,col_lname,col_phone,col_email FROM tbl_phonebook \
...
##        print("Incorrect email format!!!")
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and(len(var_email) > 0): # enforce the user to provide both names
        ...
        with conn:
            ...
            cursor.execute("\
                SELECT COUNT(col_fullname) FROM tbl_phonebook \
            ...
            if chkName == 0: # if this is 0 then there is no existance of the fullname and we can add new data
                print("chkName: {}".format(chkName))    #Testing purposes
                cursor.execute("\
                    INSERT INTO tbl_phonebook \
                    (col_fname,col_lname,col_fullname,col_phone,col_email) \
                    VALUES \
...
# Whe the user clicks the delete button
def onDelete(self):
    ...
    with conn:
        ...
        if count > 1:
            ...
            # Clicking OK makes confirm True
            if confirm:
                conn = sqlite3.connect('phonebook.db')
                with conn:
                    cursor = conn.cursor()
...
# Cleans up the text boxes when something is deleted.
def onDeleted(self):
                ...
                if response:
                    ...
                    with conn:
                        ...
                        cursor.execute("\
                            ...
                            SET col_phone = '{0}',col_email = '{1}'\
                            WHERE col_fullname = '{2}'".format(var_phone,var_email,var_value))
                        onClear(self)
                        conn.commit()
                else:
